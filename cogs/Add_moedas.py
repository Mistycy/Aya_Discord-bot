...
class Add_moedas(commands.Cog):
    ...
    async def ganhar(self, ctx, quantidade: int):
        ...
        embedVar = discord.Embed(title="Adicionar moedas 🪙", description="", color=0xffd1dc)
        ...
        embedVar.add_field(name=f"⚒️ Administrador: {ctx.author.name}", value=f"adicionou {quantidade} moedas com o saldo total de {dados[usuario_id]} moedas", inline=False)
        await ctx.send(embed=embedVar)

async def setup(bot):
    await bot.add_cog(Add_moedas(bot))