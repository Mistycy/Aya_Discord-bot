...
class Transferir_moedas(commands.Cog):
    ...
    async def transferir(self, ctx, membro: discord.Member, quantidade: int):
        ...
        if dados[usuario_id] >= quantidade:
            ...
            salvar_dados(dados)
            embedVar = discord.Embed(title="Transferir moedas 🪙", description="", color=0xffd1dc)
            ...
            embedVar.add_field(name=f"✅ O jogador {ctx.author.name}", value=f"transferiu {quantidade} moedas para o jogador {membro.name}, ficando com o saldo total de {dados[usuario_id]} moedas", inline=False)
            await ctx.send(embed=embedVar)
       
        else:
            embedVar = discord.Embed(title="Ops!! 👾", description="", color=0xffd1dc)
            embedVar.set_author(name="Aya", icon_url="https://cdn.discordapp.com/attachments/1359275472457957379/1360832965663526932/58e6c4633391436f464a5193da651966.jpg?ex=67fc8df6&is=67fb3c76&hm=f8138b24706459de1be132ddc69254b2867b7391e465b98844ed9e75bacd4541&")
            embedVar.add_field(name=f"❌ Não foi possivél ser transferido", value=f"o jogador {ctx.author.name} não tem moedas o suficiente", inline=False)
            await ctx.send(embed=embedVar)

async def setup(bot):
    await bot.add_cog(Transferir_moedas(bot))